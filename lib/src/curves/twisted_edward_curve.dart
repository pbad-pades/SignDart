...
class TwistedEdwardCurve implements Curve {
  ...
  Point mul_point(BigInt k, Point P) {
    ...
    result = _dbl_ext(x1,y1,z1,t1, q, coefficientA);
    var x2 = result[0], y2 = result[1], z2 = result[2], t2 = result[3];

    for (var i = 1; i < sz; i++) {
      if (kBits[i] == '1') {
        result = _add_ext(x2,y2,z2,t2, x1,y1,z1,t1, q, coefficientA);
        x1 = result[0]; y1 = result[1]; z1 = result[2]; t1 = result[3]; 

        result = _dbl_ext(x2,y2,z2,t2, q, coefficientA);
        x2 = result[0]; y2 = result[1]; z2 = result[2]; t2 = result[3]; 
      } else {
        result = _add_ext(x1,y1,z1,t1, x2,y2,z2,t2, q, coefficientA);
        x2 = result[0]; y2 = result[1]; z2 = result[2]; t2 = result[3];
  ...
  List _dbl_ext(BigInt X1, BigInt Y1, BigInt Z1, BigInt XY1, BigInt q, BigInt a){
    var A  = (X1*X1)%q;
    var B  = (Y1*Y1)%q;
  ...
  List _add_ext(BigInt X1,BigInt Y1,BigInt Z1,BigInt XY1,  BigInt X2,BigInt Y2,BigInt Z2,BigInt XY2, BigInt q,BigInt a){
    var A = (X1*X2)%q;
    var B = (Y1*Y2)%q;
    var C = (Z1*XY2)%q;
    var D = (XY1*Z2)%q;
